...
(function($) {
    ...
    $.fn.bPopup = function(options, callback) {
        ...
        var $popup = this,
            ...
            buffer = 200,
            popups = 0,
            id, inside, fixedVPos, fixedHPos, fixedPosStyle, vPos, hPos, height, width, debounce;
        ...
        function createContent() {
            ...
            switch (o.content) {
                ...
                default:
                    open();
                    $('<div class="b-ajax-wrapper"></div>').load(o.loadUrl, o.loadData, function() {
                        triggerCall(o.loadCallback);
                        recenter($(this));
        ...
        function open() {
            ...
            calPosition();
            $popup.data('bPopup', o).data('id', id).css({
        ...
        function close() {
            if (o.modal) {
                $('.b-modal.' + $popup.data('id')).fadeTo(o.speed, 0, function() {
                    $(this).remove();
                });
            }
            unbindEvents();
...
})((typeof(jQuery) != 'undefined') ? jQuery : window.Zepto);﻿ // Ion.Tabs
        ...
        a.ionTabs = function(c, m) {
            if (c) {
                ...
                if (b) {
                    b = b.split("|");
                    if (1 < b.length)
                        for (d = 1; d < b.length; d += 1) r = b[d].split(":"), k[decodeURIComponent(r[0])] = decodeURIComponent(r[1]);
                    b = ""
                }
                a(c).each(function() {
                    g = a(this);
...
(function($) {
    jQuery.fn.slider = function(options) {
        ...
        var $slider = $(this);
        var $slides = $slider.find(options.slides).children();
        var count = $slides.length;
        var idx = 0;
        $slider.find('[data-event]').on('click', function() {
            var event = $(this).data('event');
            switch (event) {
                case 'prev':
                    idx = $slides.filter('.active').index();
...
$(function() {
    function apply_scripts() {
        $('a[href$=".jpg"], a[href$=".JPG"], a[href$=".png"], a[href$=".gif"], a.cbox').fancybox();
        $('.slider').slider();
        ...
        var ankors = [];
        $('.toolbar a').each(function(indx, element) {
            var offset = $(this).data('offset');
            offset = offset == undefined ? -300 : offset;
            var href = $(this).attr('href');
            var top = $(href).offset().top + (offset);
            var bottom = top + $(href).outerHeight();
            ankors[indx] = [top, bottom, href];
        });
        $(window).scroll(function() {
            ...
            for (var i = 0; i < ankors.length; i++) {
                if (window_top < ankors[i][1] && window_top > ankors[i][0]) {
                    $('.toolbar a[href="' + ankors[i][2] + '"]').addClass("active");
                }
            }
        });
        $(document).on('click', '[data-tab]', function() {
            var tab = $(this).data('tab');
            $.ionTabs.setTab(tab[0], tab[1]);
        });
        ...
        $(document).on('submit', '#callback1 form', function() {
            $(this).ajaxSubmit({
                success: showResponseCallback1
            });
            return false;
        });

        function showResponseCallback1(responseText) {
            var response = $(document.createElement('div')).append(responseText).find('#callback1').html();
            $('#callback1').html(response);
        }
        $(document).on('submit', '#callback2 form', function() {
            $(this).ajaxSubmit({
                success: showResponseCallback2
            });
            return false;
        });

        function showResponseCallback2(responseText) {
            console.log(responseText);
            var response = $(document.createElement('div')).append(responseText).find('#callback2').html();
            $('#callback2').html(response);
        }
        $(document).on('submit', '#callback3 form', function() {
            $(this).ajaxSubmit({
                success: showResponseCallback3
            });
            return false;
        });

        function showResponseCallback3(responseText) {
            var response = $(document.createElement('div')).append(responseText).find('#callback3').html();
            $('#callback3').html(response);
        }
        $(document).on('submit', '#callback4 form', function() {
            $(this).ajaxSubmit({
                success: showResponseCallback4
            });
            return false;
        });

        function showResponseCallback4(responseText) {
            var response = $(document.createElement('div')).append(responseText).find('#callback4').html();
            $('#callback4').html(response);
        }
    }
    apply_scripts();
});